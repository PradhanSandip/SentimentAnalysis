...
def roberta_sentiment(review):
    ...
    encoded_input = tokenizer(review, padding=True, truncation=True, max_length=512, return_tensors="pt")
...
def process(pos1,pos2):
    ...
    for index, review in enumerate(data.get_reviews()[pos1:pos2]):
        ...
        if(not type(review) is type(float('nan'))):
            ...
            roberta = roberta_sentiment(review)
            ...
            if(ids[pos1+index] not in temp):
                ...
                if(roberta[0] == "pos"):
                    ...
                    temp[ids[pos1+index]] = [1,0,0,1,[pos1+index],[],[]]
                    ...
                    temp[ids[pos1+index]] = [0,1,0,1,[],[pos1+index],[]]
                    ...
                    temp[ids[pos1+index]] = [0,0,1,1,[],[],[pos1+index]]
            #if the movie is already in the dictionary
            else:
                #if the review is positive
                if(roberta[0] == "pos"):
                    #increment the positive counter by 1
                    temp[ids[pos1+index]][0] = temp[ids[pos1+index]][0] + 1
                    #increment the total by 1
                    temp[ids[pos1+index]][3] = temp[ids[pos1+index]][3] + 1
                    #add the positive row number to the positive list
                    temp[ids[pos1+index]][4].append(pos1+index)
                 #if the review is neutral
                elif(roberta[0] == "neu"):
                    #increment the neutral counter by 1
                    temp[ids[pos1+index]][1] = temp[ids[pos1+index]][1] + 1
                    #increment the total by 1
                    temp[ids[pos1+index]][3] = temp[ids[pos1+index]][3] + 1
                    #add the neutral row number to the neutral_list
                    temp[ids[pos1+index]][5].append(pos1+index)
                 #if the review is negative
                elif(roberta[0] == "neg"):
                    #increment the negative counter by 1
                    temp[ids[pos1+index]][2] = temp[ids[pos1+index]][2] + 1
                    #increment the total by 1
                    temp[ids[pos1+index]][3] = temp[ids[pos1+index]][3] + 1
                    #add the negative row number to the negative list
                    temp[ids[pos1+index]][6].append(pos1+index)
...
for index, thread in enumerate(threads):
    ...
    if(count == 10):
        ...
        temp.to_csv("roberta_individual.csv", sep="\t", header=["positive", "neutral", "negative", "total", "positive_list", "neutral_list", "negative_list"])
        #update user
